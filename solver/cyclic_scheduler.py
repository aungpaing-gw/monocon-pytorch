...
class CyclicScheduler(_LRScheduler):
    def __init__(
        ...
        target_momentum_ratio: Tuple[float, float] = (0.85 / 0.95, 1.0),
        period_up: float = 0.4,
    ):
        assert (
            optimizer.__class__.__name__ == "AdamW"
        ...
        # Phase 2: LR Step-Down
            ...
            return [
                self._annealing_func(
                    ...
                    (self._step_count - self.steps_up)
                    / (self.total_steps - self.steps_up),
                )
                for base_lr in self.base_lrs
            ]
