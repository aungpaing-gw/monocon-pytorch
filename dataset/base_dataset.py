...
class BaseKITTIMono3DDataset(Dataset):
    def __init__(
        ...
        self.image_files = [
            os.path.join(self.image_dir, f"{fp}.png") for fp in self.file_prefix
        ]

        # Calibration Files
        self.calib_dir = os.path.join(base_root, sub_root, "calib")
        self.calib_files = [
            os.path.join(self.calib_dir, f"{fp}.txt") for fp in self.file_prefix
        ]
        ...
        if split != "test":
            ...
            self.label_files = [
                os.path.join(self.label_dir, f"{fp}.txt") for fp in self.file_prefix
            ]
    ...
    def collect_gt_infos(self, verbose: bool = False) -> List[Dict[str, Any]]:
        ...
        for idx in iter_:
            ...
            results.append({"image": img_metas, "calib": calib_dict, "annos": obj_dict})
        return results
