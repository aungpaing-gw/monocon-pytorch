...
default_raw_transforms = [
    Normalize(
        mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], keep_origin=True
    ),
    Pad(size_divisor=32),
    ToTensor(),
    Convert_3D_to_4D(),
...
class KITTIRawDataset(Dataset):
    def __init__(self, image_dir: str, calib_file: str, img_extension: str = "png"):
        super().__init__()
        ...
        self.image_files = sorted(
            glob.glob(os.path.join(self.image_dir, rf"*.{img_extension}"))
        )
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        data_dict = {"img": img, "img_metas": metas, "calib": self.calib}
