...
class Resize3D(BaseTransform):
    ...
    def __call__(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Resize Label
        data_dict["label"]["gt_bboxes"] *= np.array(
            [*scale_hw[::-1], *scale_hw[::-1]]
        )  # 'gt_bboxes'
...
class RandomShift(BaseTransform):
    ...
    def __call__(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        if random.random() >= self.prob:
            ...
            data_dict["label"]["gt_bboxes_3d"] *= updated_mask[
                ..., np.newaxis
            ]  # (# Max Objs, 7)
            ...
            data_dict["label"]["gt_kpts_valid_mask"] *= updated_mask[
                ..., np.newaxis
            ]  # (# Max Objs, 9)
            ...
            canvas[new_y : (new_y + new_h), new_x : (new_x + new_w)] = img[
    ...
    def _shift_calib(
        self, data_dict: Dict[str, Any], sft_x: int, sft_y: int
    ) -> Dict[str, Any]:
    ...
    def _filter_kpts(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for idx, (kpt, kpt_mask, obj_mask) in enumerate(
            zip(kpts, kpts_mask, objs_mask)
        ):
            if obj_mask == 0:
                continue
...
class RandomHorizontalFlip(BaseTransform):
    ...
    def __call__(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        if random.random() >= self.prob:
            ...
            label["centers2d"][..., 0] = (w - label["centers2d"][..., 0] - 1) * label[
                "mask"
            ]
            label["gt_bboxes"] = (
                self._flip_bboxes_2d(label["gt_bboxes"], metas)
                * label["mask"][..., np.newaxis]
            )
            label["gt_bboxes_3d"] = (
                self._flip_bboxes_3d(label["gt_bboxes_3d"])
                * label["mask"][..., np.newaxis]
            )

            # Change order of 'gt_kpts_2d'
            gt_kpts_2d = label["gt_kpts_2d"].copy()
            gt_kpts_2d[..., 0::2] = (w - gt_kpts_2d[..., 0::2] - 1) * label["mask"][
                ..., np.newaxis
            ]
            max_objs, _ = gt_kpts_2d.shape

            gt_kpts_2d = gt_kpts_2d.reshape(max_objs, -1, 2)
            gt_kpts_2d[:, [0, 1, 2, 3, 4, 5, 6, 7]] = gt_kpts_2d[
                :, [1, 0, 3, 2, 5, 4, 7, 6]
            ]
            ...
            gt_kpts_valid_mask[:, [0, 1, 2, 3, 4, 5, 6, 7]] = gt_kpts_valid_mask[
                :, [1, 0, 3, 2, 5, 4, 7, 6]
            ]
    ...
    def _flip_bboxes_2d(
        self, bboxes_2d: np.ndarray, img_metas: Dict[str, Any]
    ) -> np.ndarray:
        # 'bboxes_2d': (B, K, 4)
