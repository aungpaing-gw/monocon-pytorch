...
class RandomCrop3D(BaseTransform):
    ...
    def __call__(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        assert (
            sum(np.array(self.crop_size) <= np.array(img_shape)) == 2
        ...
        if not is_cropped:
            ...
            for idx, (gt_bbox, mask) in enumerate(zip(gt_bboxes, ori_mask)):
                ...
                rel_type, coord = self._compare_two_area(
                    frame_bbox=np.array(crop_coord), gt_bbox=gt_bbox
                )
    ...
    def _filter_kpts(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        valid_region = data_dict["img_metas"][
            "crop_coord"
        ...
        # 'gt_kpt'      : (18,)
        # 'gt_kpt_mask' : (9,)
        for obj_idx, (gt_kpt, gt_kpt_mask, obj_mask) in enumerate(
            zip(gt_kpts_2d, gt_kpts_valid_mask, obj_masks)
        ):
            if obj_mask == 0:
                continue
    ...
    def _compare_two_area(
        self, frame_bbox: np.ndarray, gt_bbox: np.ndarray
    ) -> Tuple[str, Any]:
