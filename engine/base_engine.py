...
class BaseEngine:
    def __init__(
        self, cfg: Union[str, CfgNode], auto_resume: bool = True, is_test: bool = False
    ):
        # Configuration
        ...
        if not is_test:
            ...
            if os.path.isdir(self.weight_dir) and auto_resume:
                ...
                if len(pth_files) > 0:
                    ...
                    tprint(
                        f"Existing checkpoint '{latest_weight}' is found and loaded automatically."
                    )
    ...
    def train(self, resume_from: str = None) -> None:
        ...
        if resume_from is not None:
            ...
            tprint(
                f"Training resumes from '{resume_from}'. (Start Epoch: {self.epochs})"
            )

        # Start Training
        tprint(
            f"Training will be proceeded from epoch {self.epochs} to epoch {self.target_epochs}."
        )
    ...
    def save_checkpoint(
        self, post_fix: str = None, save_after_update: bool = True, verbose: bool = True
    ) -> None:
    ...
    def load_checkpoint(self, ckpt_file: str, verbose: bool = False) -> None:
    ...
    def _update_dict_to_writer(
        self, data: Dict[str, Union[torch.Tensor, float]], tag: str
    ):
    ...
    def _print_engine_info(self):
        ...
        print(f"\n- Seed: {self.cfg.SEED}")
        print(
            f"- Device: GPU {self.cfg.GPU_ID} ({torch.cuda.get_device_name(self.cfg.GPU_ID)})"
        )
