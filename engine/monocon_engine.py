...
class MonoconEngine(BaseEngine):
    ...
    def build_loader(self, is_train: bool = True):
        dataset = MonoConDataset(
            ...
            filter_configs={
                k.lower(): v for k, v in dict(self.cfg.DATA.FILTER).items()
            },
        )
    ...
    def train_one_epoch(self) -> float:
        ...
        for batch_idx, data_dict in enumerate(self.train_loader):
            ...
            if self.cfg.SOLVER.CLIP_GRAD.ENABLE:
                clip_args = {
                    k.lower(): v
                    for k, v in dict(self.cfg.SOLVER.CLIP_GRAD).items()
                    if k not in ["ENABLE"]
                }
            ...
            if self.global_iters % self.log_period == 0:
                ...
                prog_bar = progress_to_string_bar(
                    (batch_idx + 1), one_epoch_steps, bins=20
                )
                recent_loss = sum(self.entire_losses[-100:]) / len(
                    self.entire_losses[-100:]
                )
                print(
                    f"| Progress {prog_bar} | LR {self.current_lr:.6f} | Loss {total_loss.item():8.4f} ({recent_loss:8.4f}) |"
                )
    ...
    def evaluate(self) -> Dict[str, float]:
        ...
        eval_container = {"img_bbox": [], "img_bbox2d": []}
        ...
        eval_dict = self.test_dataset.evaluate(
            eval_container, eval_classes=["Pedestrian", "Cyclist", "Car"], verbose=True
        )
    ...
    @torch.no_grad()
    def visualize(self, output_dir: str, draw_items: List[str] = ["bev", "2d", "3d"]):
        cvt_flag = False
        if self.model.training:
            self.model.eval()
            cvt_flag = True
        ...
        visualizer = Visualizer(
            self.test_dataset, vis_format=vis_container, scale_hw=scale_hw
        )
        draw_item_to_func = {
            "2d": "plot_bboxes_2d",
            "3d": "plot_bboxes_3d",
        ...
        for draw_item in draw_items:
            ...
            for idx in tqdm(
                ...
                desc=f"Visualizing '{draw_item.upper()}'...",
            ):
                draw_func = getattr(visualizer, draw_item_to_func[draw_item])

                ori_filename = os.path.basename(
                    self.test_dataset[idx]["img_metas"]["image_path"]
                )
