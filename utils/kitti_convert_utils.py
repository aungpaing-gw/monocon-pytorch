...
def get_valid_bboxes_3d(
    result_3d: Dict[str, torch.Tensor],
...
def convert_to_kitti_3d(
    results_3d: List[Dict[str, torch.Tensor]],
    img_metas: Dict[str, Any],
    calibs: List[KITTICalibration],
) -> List[Dict[str, Any]]:
    returns = []
    ...
    for batch_idx, result_3d in enumerate(results_3d):
        ...
        if len(valid_box_dict["bbox"]) > 0:
            ...
            for box, bbox, score, label in zip(
                box_preds, box_2d_preds, scores, label_preds
            ):
...
def convert_to_kitti_2d(
    results_2d: List[List[np.ndarray]], img_metas: Dict[str, Any]
) -> List[Dict[str, Any]]:
    ...
    for batch_idx, result_2d in enumerate(results_2d):
        ...
        annos[-1]["sample_idx"] = np.array([sample_idx] * num_objs, dtype=np.int64)
        returns.extend(annos)
