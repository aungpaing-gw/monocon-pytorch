...
class TargetGenerator:
    ...
    def __call__(
        self, input_dict: Dict[str, Any], feat_shape: Tuple[int]
        ...
        for b_idx in range(batch_size):
            ...
            bbox_ctx = (bboxes[:, 0] + bboxes[:, 2]) * w_ratio / 2.0
            bbox_cty = (bboxes[:, 1] + bboxes[:, 3]) * h_ratio / 2.0
            ...
            for o_idx, ct in enumerate(bbox_ct):
                ...
                target["wh_target"][b_idx, o_idx] = torch.Tensor(
                    [feat_box_w, feat_box_h]
                )
                target["offset_target"][b_idx, o_idx] = torch.Tensor(
                    [(ctx - ctx_int), (cty - cty_int)]
                )
                ...
                for k_idx in range(self.num_kpt):
                    ...
                    target["center2kpt_offset_target"][b_idx, o_idx, (k_idx * 2)] = (
                        kptx - ctx_int
                    )
                    ...
                    is_kpt_inside_feat = (0 <= kptx_int < feat_w) and (
                        0 <= kpty_int < feat_h
                    )
                    ...
                    generate_gaussian_target(
                        target["kpt_heatmap_target"][b_idx, k_idx],
                        center=[kptx_int, kpty_int],
                        radius=target_radius,
                    )

                    target["indices_kpt"][b_idx, o_idx, k_idx] = (
                        kpty_int * feat_w + kptx_int
                    )

                    target["kpt_heatmap_offset_target"][b_idx, o_idx, (k_idx * 2)] = (
                        kptx - kptx_int
                    )
    ...
    def _create_empty_target(
        self, feat_shape: Tuple[int], device: str = None
    ) -> Dict[str, torch.Tensor]:
        batch_size, _, feat_h, feat_w = feat_shape
        container = {
            ...
            "center2kpt_offset_target": torch.zeros(
                (batch_size, self.max_objs, self.num_kpt * 2)
            ),
            "kpt_heatmap_target": torch.zeros(
                (batch_size, self.num_kpt, feat_h, feat_w)
            ),
            "kpt_heatmap_offset_target": torch.zeros(
                (batch_size, self.max_objs, self.num_kpt * 2)
            ),
            "indices": torch.zeros((batch_size, self.max_objs)).type(torch.LongTensor),
            "indices_kpt": torch.zeros((batch_size, self.max_objs, self.num_kpt)).type(
                torch.LongTensor
            ),
            "mask_target": torch.zeros((batch_size, self.max_objs)),
            "mask_center2kpt_offset": torch.zeros(
                (batch_size, self.max_objs, self.num_kpt * 2)
            ),
            "mask_kpt_heatmap_offset": torch.zeros(
                (batch_size, self.max_objs, self.num_kpt * 2)
            ),
        }
