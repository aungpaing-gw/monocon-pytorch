...
class KITTICalibration:
    ...
    def get_info_dict(self) -> Dict[str, Any]:
        return {
            "P0": self._convert_to_4x4(self.P0),
...
class KITTISingleObject:
    def __init__(self, parsed_line: str, calib: KITTICalibration):
        ...
        cls_str_to_idx = {"DontCare": -1, "Pedestrian": 0, "Cyclist": 1, "Car": 2}
    ...
    def translate(
        self, shift_x: float, shift_y: float = 0.0, shift_z: float = 0.0
    ) -> None:
    ...
    def projected_kpts(self) -> np.ndarray:
        ...
        all_corner_coords = view_points(
            original_corners_3d, self.calib.P2, normalize=True
        ).T[:, :2]
        all_corner_coords = np.hstack([all_corner_coords, valid_corners_mask])

        proj_center[2] = 1
        projected_pts = np.concatenate(
            [all_corner_coords, proj_center[np.newaxis, ...]]
        )
...
class KITTIMultiObjects:
    def __init__(self, obj_list: List[KITTISingleObject], ignore_dontcare: bool = True):
