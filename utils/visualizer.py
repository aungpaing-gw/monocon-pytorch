...
class Visualizer:
    def __init__(
        ...
        if scale_hw is None:
            scale_hw = np.array([1.0, 1.0])
        self.scale_hw = scale_hw

        # Mode
        self.mode = (
            "raw" if (dataset.__class__.__name__ == "KITTIRawDataset") else "normal"
        )

    def get_labels(
        self, idx: int, search_key: Union[List[str], str]
    ) -> List[np.ndarray]:
        assert self.mode == "normal", "This method is only available in 'normal' mode."
    ...
    def plot_bboxes_2d(
        self, idx: int, save_path: str = None
        ...
        for c_idx, pred_bbox in enumerate(pred_bboxes):
            ...
            for box in pred_bbox:
                s = np.reciprocal(
                    np.array([*self.scale_hw[::-1], *self.scale_hw[::-1]])
                )
    ...
    def plot_bboxes_3d(
        ...
        if len(pred_bboxes_3d) > 0:
            ...
            for bbox_3d, label_3d in zip(pred_bboxes_3d, pred_labels_3d):
                corners = extract_corners_from_bboxes_3d(bbox_3d.unsqueeze(0))[
                    0
                ]  # (8, 3)
    ...
    def plot_bev(self, idx: int, save_path: str = None) -> Union[None, np.ndarray]:
        ...
        if len(pred_bboxes_3d) > 0:
            ...
            for idx, bev in enumerate(pred_bev):
                ...
                space = cv2.drawContours(
                    space, [box], -1, color, thickness=-1, lineType=cv2.LINE_AA
                )
    ...
    def export_as_video(
        self, save_dir: str, plot_items: List[str] = ["2d", "3d", "bev"], fps: int = 20
    ) -> None:
        ...
        for plot_item in plot_items:
            ...
            for idx in tqdm(
                range(len(self.dataset)), desc=f"Visualizing '{plot_item}'..."
            ):
            ...
            vid_writer = cv2.VideoWriter(
                vid_file, cv2.VideoWriter_fourcc(*"mp4v"), fps, img_size
            )
    ...
    def _add_transparent_box(
        ...
        ori_image = cv2.rectangle(
            ori_image, (x1, y1), (x2, y2), color, thickness=2, lineType=cv2.LINE_AA
        )
