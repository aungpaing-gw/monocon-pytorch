...
class MonoConDenseHeads(nn.Module):
    def __init__(
        ...
        self.center2kpt_offset_head = self._build_head(
            in_ch, feat_ch, (self.num_kpts * 2)
        )
    ...
    def init_weights(self, prior_prob: float = 0.1) -> None:
        ...
        for head in [
            self.wh_head,
            self.offset_head,
            self.center2kpt_offset_head,
            self.kpt_heatmap_offset_head,
            self.depth_head,
            self.dim_head,
            self.dir_feat,
            self.dir_cls,
    ...
    def forward_train(
        self, feat: torch.Tensor, data_dict: Dict[str, Any]
    ...
    def _get_predictions(self, feat: torch.Tensor) -> Dict[str, torch.Tensor]:
        # (1) HeatMap
        heat_min, heat_max = 1e-4, (1.0 - 1e-4)
        center_heatmap_pred = torch.clamp(
            torch.sigmoid(self.heatmap_head(feat)), heat_min, heat_max
        )
        kpt_heatmap_pred = torch.clamp(
            torch.sigmoid(self.kpt_heatmap_head(feat)), heat_min, heat_max
        )
        ...
        depth_pred[:, 0, :, :] = (
            1.0 / (torch.sigmoid(depth_pred[:, 0, :, :]) + EPS)
        ) - 1
    ...
    def _get_losses(
        ...
        ct_heat_loss = self.crit_center_heatmap(
            pred_dict["center_heatmap_pred"], target_dict["center_heatmap_target"]
        )
        kpt_heat_loss = self.crit_kpt_heatmap(
            pred_dict["kpt_heatmap_pred"], target_dict["kpt_heatmap_target"]
        )
        ...
        center2kpt_offset_pred = extract_input(
            pred_dict["center2kpt_offset_pred"], indices, mask_target
        )
        center2kpt_offset_target = extract_target(
            target_dict["center2kpt_offset_target"], mask_target
        )
        mask_center2kpt_offset = extract_target(
            target_dict["mask_center2kpt_offset"], mask_target
        )

        center2kpt_offset_pred = center2kpt_offset_pred * mask_center2kpt_offset
        center2kpt_offset_loss = self.crit_center2kpt_offset(
            center2kpt_offset_pred,
            center2kpt_offset_target,
            avg_factor=(mask_center2kpt_offset.sum() + EPS),
        )

        kpt_heatmap_offset_pred = transpose_and_gather_feat(
            pred_dict["kpt_heatmap_offset_pred"], indices_kpt
        )
        kpt_heatmap_offset_pred = kpt_heatmap_offset_pred.reshape(
            batch_size, self.max_objs, (self.num_kpts * 2)
        )
        kpt_heatmap_offset_pred = extract_target(kpt_heatmap_offset_pred, mask_target)

        kpt_heatmap_offset_target = extract_target(
            target_dict["kpt_heatmap_offset_target"], mask_target
        )
        mask_kpt_heatmap_offset = extract_target(mask_kpt_heatmap_offset, mask_target)

        kpt_heatmap_offset_loss = self.crit_kpt_heatmap_offset(
        ...
        # Bin Classification
        alpha_cls_pred = extract_input(
            pred_dict["alpha_cls_pred"], indices, mask_target
        )
        alpha_cls_target = extract_target(
            target_dict["alpha_cls_target"], mask_target
        ).type(torch.LongTensor)
        alpha_cls_onehot_target = (
            alpha_cls_target.new_zeros([len(alpha_cls_target), self.num_alpha_bins])
            .scatter_(1, alpha_cls_target.view(-1, 1), 1)
            .to(device)
        )

        if mask_target.sum() > 0:
            loss_alpha_cls = self.crit_alpha_cls(
                alpha_cls_pred, alpha_cls_onehot_target
            )
        else:
            loss_alpha_cls = 0.0

        # Bin Offset Regression
        alpha_offset_pred = extract_input(
            pred_dict["alpha_offset_pred"], indices, mask_target
        )
        alpha_offset_pred = torch.sum(
            alpha_offset_pred * alpha_cls_onehot_target, 1, keepdim=True
        )
        alpha_offset_target = extract_target(
            target_dict["alpha_offset_target"], mask_target
        )
    ...
    def _get_bboxes(
        self, data_dict: Dict[str, Any], pred_dict: Dict[str, torch.Tensor]
    ) -> Tuple[torch.Tensor]:
    ...
    def _get_eval_formats(
        self,
        data_dict: Dict[str, Any],
        pred_dict: Dict[str, torch.Tensor],
        get_vis_format: bool = False,
    ) -> Dict[str, Any]:
        ...
        kitti_3d = convert_to_kitti_3d(
            collected_3d, data_dict["img_metas"], data_dict["calib"]
        )

        kitti_format = {"img_bbox": kitti_3d, "img_bbox2d": kitti_2d}
        return kitti_format

    def decode_alpha(
        self, alpha_cls: torch.Tensor, alpha_offset: torch.Tensor
    ) -> torch.Tensor:
    ...
    def decode_heatmap(
        ...
        center_heatmap_pred = get_local_maximum(
            center_heatmap_pred, kernel=self.local_maximum_kernel
        )

        # (B, K)
        scores, indices, topk_labels, ys, xs = get_topk_from_heatmap(
            center_heatmap_pred, k=self.topk
        )

        # Get 2D Predictions from Predicted Heatmap
        wh = transpose_and_gather_feat(pred_dict["wh_pred"], indices)  # (B, K, 2)
        offset = transpose_and_gather_feat(
            pred_dict["offset_pred"], indices
        )  # (B, K, 2)

        topk_xs = xs + offset[..., 0]
        topk_ys = ys + offset[..., 1]

        x1 = (topk_xs - wh[..., 0] / 2.0) * (img_w / feat_w)
        y1 = (topk_ys - wh[..., 1] / 2.0) * (img_h / feat_h)
        x2 = (topk_xs + wh[..., 0] / 2.0) * (img_w / feat_w)
        y2 = (topk_ys + wh[..., 1] / 2.0) * (img_h / feat_h)
        ...
        alpha_cls = transpose_and_gather_feat(
            pred_dict["alpha_cls_pred"], indices
        )  # (B, K, 12)
        alpha_offset = transpose_and_gather_feat(
            pred_dict["alpha_offset_pred"], indices
        )  # (B, K, 12)
        alpha = self.decode_alpha(alpha_cls, alpha_offset)  # (B, K, 1)

        depth_pred = transpose_and_gather_feat(
            pred_dict["depth_pred"], indices
        )  # (B, K, 2)
        sigma = torch.exp(-depth_pred[:, :, 1])  # (B, K)
        bboxes_2d[..., -1] = bboxes_2d[..., -1] * sigma

        center2kpt_offset = transpose_and_gather_feat(
            pred_dict["center2kpt_offset_pred"], indices
        )
        ...
        center2kpt_offset[..., 1::2] = (
            center2kpt_offset[..., 1::2] + y_offset
        ) * y_scale

        center2d = center2kpt_offset
        rot_y = self.calculate_roty(
            center2d, alpha, batched_calib=data_dict["calib"]
        )  # (B, K, 1)

        depth = depth_pred[:, :, 0:1]  # (B, K, 1)
        center3d = torch.cat([center2d, depth], dim=-1)  # (B, K, 3)
        center3d = self.convert_pts2D_to_pts3D(
            center3d, batched_calib=data_dict["calib"]
        )  # (B, K, 3)
        ...
        # Decoded Results
        ret_bboxes_2d = [bbox_2d[mask] for bbox_2d, mask in zip(bboxes_2d, box_mask)]

        ret_bboxes_3d = [bbox_3d[mask] for bbox_3d, mask in zip(bboxes_3d, box_mask)]

        ret_labels = [label[mask] for label, mask in zip(topk_labels, box_mask)]
    ...
    def calculate_roty(
        ...
        kpts: torch.Tensor,
        alpha: torch.Tensor,
        batched_calib: List[KITTICalibration],
    ) -> torch.Tensor:
        """
        * Args:
            - 'kpts'
        ...
        proj_matrices = torch.cat(
            [torch.from_numpy(calib.P2).unsqueeze(0) for calib in batched_calib], dim=0
        )
    ...
    def convert_pts2D_to_pts3D(
        self, points_2d: torch.Tensor, batched_calib: List[KITTICalibration]
    ) -> torch.Tensor:
        """
        * Args:
            - 'points_2d'
        ...
        for b_idx, unnorm_point in enumerate(unnorm_points):
            ...
            viewpad = torch.eye(4)
            viewpad[: proj_mat.shape[0], : proj_mat.shape[1]] = points_2d.new_tensor(
                proj_mat
            )
            ...
            homo_points = torch.cat(
                [unnorm_point, points_2d.new_ones((nbr_points, 1))], dim=1
            )  # (K, 4)
            points_3d = torch.mm(homo_points, inv_viewpad)[
                :, :3
            ]  # (K, 4) * (4, 4) = (K, 4) -> (K, 3)
    ...
    def bbox_2d_to_result(
        self, bboxes_2d: torch.Tensor, labels: torch.Tensor, num_classes: int
    ) -> List[np.ndarray]:
    ...
    def bbox_3d_to_result(
        self, bboxes_3d: torch.Tensor, scores: torch.Tensor, labels: torch.Tensor
    ) -> Dict[str, torch.Tensor]:
        result_dict = dict(
            boxes_3d=bboxes_3d.cpu(), scores_3d=scores.cpu(), labels_3d=labels.cpu()
        )
