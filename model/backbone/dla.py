...
class Tree(nn.Module):
    ...
    def forward(self, x: torch.Tensor, residual=None, children=None):
...
class DLA(nn.Module):
    ...
    def __init__(self, num_layers: int, in_channels: int = 3, pretrained: bool = True):
        super().__init__()

        assert (
            num_layers in self.arch_settings.keys()
        ...
        self.level0 = self._make_multilevel_conv(
            channels[0], channels[0], num_levels=levels[0]
        )
        self.level1 = self._make_multilevel_conv(
            channels[0], channels[1], num_levels=levels[1], stride=2
        )
        self.level2 = Tree(
            levels[2],
            block,
